...
def configure_mps_runtime_env(env: dict[str, str] | None = None) -> dict[str, str]:
    runtime_env = os.environ if env is None else env
    runtime_env.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")
    return runtime_env
...
class TorchMpsBackend:
    ...
    def __init__(self, config: InferenceConfig) -> None:
        if not config.model_path:
            raise ValueError("model_path is required for the MPS backend")

        configure_mps_runtime_env()

        try:
            import torch
            from ultralytics import YOLO
        except ImportError as exc:
            raise ImportError("Missing dependencies for the MPS backend") from exc
